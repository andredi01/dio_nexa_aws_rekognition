...
def compare_faces(source_image_path: str, target_image_path: str, similarity_threshold: int = 80):
...
def draw_boxes(image_path: str, output_path: str, face_details: list[CompareFacesMatchTypeDef]) -> None:
...
if __name__ == "__main__":
    ...
    target_images = ["bale.jpg", "bbc.jpg", "cr7.jpg", "messi.jpg", "msn.jpg"]

    for target_file in target_images:
        target_image_path = get_path(target_file)
        output_image_path = get_path(f"resultado_{target_file}")

        print(f"\nComparando Neymar com {target_file}...")
